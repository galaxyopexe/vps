...
async def tips_command(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(embed=embed, view=view)
@bot.tree.command(name="delvps", description="🗑️ Delete a VPS instance by name")
...
async def delvps(interaction: discord.Interaction, container_name: str):
    user = str(interaction.user)
    is_admin_user = is_admin(interaction.user.id)

    if not os.path.exists(database_file):
        embed = discord.Embed(
            title="❌ Not Found",
            description="No VPS database found.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    if not match:
        embed = discord.Embed(
            title="❌ Not Found",
            description=f"No VPS named `{container_name}` found in the system.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    if (owner != user) and not is_admin_user:
        embed = discord.Embed(
            title="🚫 Access Denied",
            description=f"You do not own VPS `{container_name}`, and you are not an admin.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    confirm_embed = discord.Embed(
        ...
        description=f"Are you sure you want to delete VPS `{container_name}`? This cannot be undone.",
        color=0xff9900
    )
    view = ConfirmView(container_name, container_name)
...
async def sendvps(
    ...
    embed.set_footer(text="🔐 Safe your details | Powered by NXH-i9")
