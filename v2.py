...
async def tips_command(interaction: discord.Interaction):
    ...
    await interaction.response.send_message(embed=embed, view=view)
@bot.tree.command(name="listadmin", description="📋 Admin: List all VPS container names (up to 200)")
async def list_all_servers(interaction: discord.Interaction):
    ...
    servers = get_all_containers()
    await interaction.response.defer()

    if not servers:
        embed = discord.Embed(
            title="📋 All VPS Instances",
            description="No VPS instances found.",
            color=0x00aaff
        )
        await interaction.followup.send(embed=embed)
        return
    ...
    embed = discord.Embed(
        title="📋 All VPS Container Names",
        description=f"Total VPS (showing up to 200): {len(servers)}",
        color=0x00aaff
    )
    ...
    # Join names into one code block
    if container_list:
        container_text = "\n".join(container_list)
        embed.add_field(
            name="🖥️ Container Names",
            value=f"```{container_text}```",
            inline=False
        )
...
async def delete_server(interaction: discord.Interaction, container_name: str):
    ...
    # Check if this VPS belongs to the user
    match = None
    for server in servers:
        parts = server.split('|')
    ...
    if not match:
        embed = discord.Embed(
            title="❌ Not Found",
            description=f"No VPS named `{container_name}` found for your account.",
            color=0xff0000
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    confirm_embed = discord.Embed(
        ...
        description=f"Are you sure you want to delete VPS `{container_name}`? This action cannot be undone.",
        color=0xff9900
    )

    view = ConfirmView(container_name, container_name)  # pass container_name instead of ID
    await interaction.response.send_message(embed=confirm_embed, view=view)


# Update ConfirmView to work with container_name
class ConfirmView(View):
    def __init__(self, container_id, container_name, is_delete_all=False):
        super().__init__(timeout=60)
        self.container_id = container_id
    ...
    async def confirm_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            if self.is_delete_all:
                ...
                for container_info in containers:
                    ...
                    if len(parts) >= 2:
                        cname = parts[1]
                        try:
                            subprocess.run(["docker", "stop", cname], check=True, stderr=subprocess.DEVNULL)
                            subprocess.run(["docker", "rm", cname], check=True, stderr=subprocess.DEVNULL)
                            remove_from_database(cname)
                            deleted_count += 1
                        except Exception:
                            pass
                with open(database_file, 'w') as f:
                ...
                # Single VPS delete
                subprocess.run(["docker", "stop", self.container_name], check=True, stderr=subprocess.DEVNULL)
                subprocess.run(["docker", "rm", self.container_name], check=True, stderr=subprocess.DEVNULL)
                remove_from_database(self.container_name)

                embed = discord.Embed(
                    title="🗑️ VPS Deleted",
                    description=f"VPS `{self.container_name}` was deleted successfully.",
                    color=0x00ff00
                )
                await interaction.followup.send(embed=embed)
            ...
            embed = discord.Embed(
                title="❌ Error",
                description=f"Failed to delete VPS: {str(e)}",
                color=0xff0000
            )
            await interaction.followup.send(embed=embed)
    ...
    async def cancel_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed(
            title="🚫 Cancelled",
            description="The delete operation was cancelled.",
            color=0xffcc00
        )
        await interaction.followup.send(embed=embed)
        for child in self.children:
